...
def line_seg(bw):
    ...
    segment1 = []
    for i in range(0, len(rows_sum) - 1):
        if rows_sum[i + 1] < 1158:
            segment1.append(i)
    print(segment1)

    segment = []
    for i in range(0, len(rows_sum) - 1):
        if rows_sum[i] == col_num and rows_sum[i + 1] < col_num:
            segment.append(i - 1)
        if rows_sum[i] < col_num and rows_sum[i + 1] == col_num:
            segment.append(i + 1)
    print(segment)
...
for i1 in range(0, len(line)):
    ...
    print("VALUE")
    print(lena, lenac)
    for i in range(0, len(cols_sum) - 1):
        if cols_sum[i] > lena - 4 and (cols_sum[i + 1]) <= lena - 4 and newsymbol == 0:
            segment2.append(i)
            newsymbol = 1
        if cols_sum[i] < lena - 1 and newsymbol == 1 and cols_sum[i + 1] > lena - 4:
            segment2.append(i + 1)
            newsymbol = 0
        print(i, cols_sum[i])
    print(lena, lenac)
    print("segment")
    print(segment2)
    # 272, 298, 296, 311, 308, 323, 322, 336, 334, 345, 346, 361, 358, 373, 372, 397, 397, 411, 410, 437, 435, 449,
    # 448, 462, 460, 474, 472, 486, 484, 498, 498, 511, 509, 523, 521, 535, 534, 547, 547, 560
